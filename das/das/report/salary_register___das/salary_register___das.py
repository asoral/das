...
def execute(filters=None):
	...
	for ss in salary_slips:
		row = [ss.name, ss.employee, ss.employee_name, doj_map.get(ss.employee), ss.branch, ss.department, ss.designation,
			ss.company, ss.start_date, ss.end_date, ss.total_working_days, ss.leave_without_pay, ss.payment_days,ss.cl_balance,ss.el_balance]
		...
		if not ss.total_working_days  == None: columns[9] = columns[9].replace('-1','130')
		if not ss.leave_without_pay  == None: columns[9] = columns[9].replace('-1','120')
		if not ss.cl_balance  == None: columns[11] = columns[11].replace('-1','100')
		if not ss.el_balance  == None: columns[12] = columns[12].replace('-1','100')
...
def get_columns(salary_slips):
	columns = [
		...
		_("End Date") + "::80", _("Total Working Days") + ":Float:120", _("Leave Without Pay") + ":Float:120", _("Payment Days") + ":Float:120",
		 _("CL Balance") + ":Float:80", _("EL Balance") + ":Float:80"
	]
