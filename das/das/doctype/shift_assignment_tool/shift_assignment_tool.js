...
frappe.ui.form.on('Shift Assignment Tool', {
	...
	emp_category: function(frm){
		frm.clear_table("shift_assignment_tool_kit")
		refresh_field("shift_assignment_tool_kit")
		if (frm.doc.emp_category) {
			frappe.call({
				"method": "frappe.client.get_list",
				args: {
					"doctype": "Employee",
					filters: {
						 "emp_category": frm.doc.emp_category
					},
	...
	assign: function (frm, cdt, cdn) {
		if(!frm.doc.from_date){
			...
			frappe.call({
				"method": "das.das.doctype.shift_assignment_tool.shift_assignment_tool.update_shift",
				args: {
